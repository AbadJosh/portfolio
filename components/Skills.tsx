...
const categories = [
  { title: "Data Engineering",        label: "Data Eng",  color: "#3b82f6", Icon: Layers,   skills: ["Apache Kafka","Apache Spark","Apache Airflow","dbt","ETL / ELT Design","Data Modeling","Stream Processing","Power BI"] },
  { title: "Cloud & Infrastructure",  label: "Cloud",     color: "#8b5cf6", Icon: Cloud,    skills: ["Microsoft Azure","AWS","Google Cloud","Terraform","Azure DevOps","CI/CD Pipelines","Infrastructure as Code"] },
  { title: "Databases",               label: "Databases", color: "#06b6d4", Icon: Database, skills: ["PostgreSQL","Snowflake","BigQuery","AWS Redshift","SQL Server","Azure SQL","Redis","MySQL"] },
  { title: "Languages & Frameworks",  label: "Languages", color: "#10b981", Icon: Code2,    skills: ["Python","SQL / T-SQL","TypeScript","JavaScript","Node.js","Angular","REST APIs","Event-Driven APIs"] },
  { title: "Quality & Observability", label: "Quality",   color: "#f59e0b", Icon: Activity, skills: ["Great Expectations","Data Contracts","Pipeline Monitoring","Data Lineage","dbt Tests","App Insights","Schema Governance"] },
];
...
export default function Skills() {
  ...
  return (
    <section id="skills" style={{ background: "var(--th-bg)", padding: "100px 0" }}>
      <style>{`
        .sk-node { cursor:pointer; background:none; border:none; padding:0; position:absolute; }
        .sk-node:focus-visible { outline:2px solid var(--th-blue); border-radius:50%; }
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "0 24px" }} ref={ref}>
        ...
        >
          <div className="sk-penta" style={{ position: "relative", width: SVG_W, height: SVG_H, margin: "0 auto" }}>
            <svg
            ...
            >
              ...
              {starLines.map(([a, b], i) => (
                <line key={i} x1={pts[a].x} y1={pts[a].y} x2={pts[b].x} y2={pts[b].y}
                  stroke="var(--th-sk-star)" strokeWidth="1" />
              ))}

              {/* Active polygon edges */}
              {[0,1,2,3,4].map(i => {
                const j = (i + 1) % 5;
                if (i !== active && j !== active) return null;
                return <line key={`ae${i}`} x1={pts[i].x} y1={pts[i].y} x2={pts[j].x} y2={pts[j].y}
                  stroke={cat.color} strokeWidth="1.5" opacity="0.75" filter="url(#sk-glow)" />;
              })}
              ...
              {starLines.map(([a, b], i) => {
                ...
                return <line key={`as${i}`} x1={pts[a].x} y1={pts[a].y} x2={pts[b].x} y2={pts[b].y}
                  stroke={cat.color} strokeWidth="1" opacity="0.45" filter="url(#sk-glow)" />;
              })}
              ...
              return (
                ...
                >
                  ...
                  <div style={{
                    ...
                    background: on
                      ? `radial-gradient(circle at 38% 35%, ${c.color}28 0%, var(--th-sk-node-active-bg) 65%)`
                      : "var(--th-sk-node-inactive-fill)",
                    border: `2px solid ${on ? c.color : "var(--th-sk-node-inactive-border)"}`,
                    boxShadow: on
                      ? `0 0 18px ${c.color}dd, 0 0 48px ${c.color}66, 0 0 90px ${c.color}22, inset 0 0 20px ${c.color}12`
                      : "var(--th-sk-node-inactive-shadow)",
                    ...
                    <Icon
                      size={22}
                      style={{
                        color: on ? c.color : "var(--th-sk-node-inactive-icon)",
